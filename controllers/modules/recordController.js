...
const { recordService: { getOnesAllRecord, updateRecord, createRecord, deleteRecord } } = require('../../services')
const recordController = {
  getRecord: async (req, res, next) => {
    try {
      const userId = req.user.id
      const records = await getOnesAllRecord(userId)
      const message = 'get all record success,data->all record of user id ' + userId
  ...
  postRecord: async (req, res, next) => {
    try {
      ...
      const userId = req.user.id
      const postData = await createRecord({ name, date, amount, categoryId, userId })
      return res.json({ status: true, message: 'post record success,data->post record', postData })
    } catch (error) {
      console.log(error)
      return next(error)
    }
